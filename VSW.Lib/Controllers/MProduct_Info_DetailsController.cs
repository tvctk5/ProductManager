...
{
    [ModuleInfo(Name = "MO : Product_ info_ details", Code = "MProduct_Info_Details", Order = 50)]
    public class MProduct_Info_DetailsController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Info_DetailsModel model)
        {
        ...
        {
            var item = ModProduct_Info_DetailsService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Info_DetailsService.Instance.CreateQuery()
    ...
    public class MProduct_Info_DetailsModel
    {
        private int _Page = 0;
        public int Page
        {
