...
{
    [ModuleInfo(Name = "MO : D t_ dai ly", Code = "MDT_DaiLy", Order = 50)]
    public class MDT_DaiLyController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MDT_DaiLyModel model)
        {
        ...
        {
            var item = ModDT_DaiLyService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModDT_DaiLyService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
