...
{
    [ModuleInfo(Name = "MO : Product_ slide show", Code = "MProduct_SlideShow", Order = 50)]
    public class MProduct_SlideShowController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_SlideShowModel model)
        {
        ...
        {
            var item = ModProduct_SlideShowService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_SlideShowService.Instance.CreateQuery()
    ...
    public class MProduct_SlideShowModel
    {
        private int _Page = 0;
        public int Page
        {
