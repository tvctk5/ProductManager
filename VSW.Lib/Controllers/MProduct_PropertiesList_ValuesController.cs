...
{
    [ModuleInfo(Name = "MO : Product_ properties list_ values", Code = "MProduct_PropertiesList_Values", Order = 50)]
    public class MProduct_PropertiesList_ValuesController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_PropertiesList_ValuesModel model)
        {
        ...
        {
            var item = ModProduct_PropertiesList_ValuesService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_PropertiesList_ValuesService.Instance.CreateQuery()
    ...
    public class MProduct_PropertiesList_ValuesModel
    {
        private int _Page = 0;
        public int Page
        {
