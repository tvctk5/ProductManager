...
{
    [ModuleInfo(Name = "MO : Ward", Code = "MWard", Order = 50)]
    public class MWardController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MWardModel model)
        {
        ...
        {
            var item = ModWardService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModWardService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
