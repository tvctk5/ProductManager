...
{
    [ModuleInfo(Name = "MO : Product_ filter values", Code = "MProduct_FilterValues", Order = 50)]
    public class MProduct_FilterValuesController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_FilterValuesModel model)
        {
        ...
        {
            var item = ModProduct_FilterValuesService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_FilterValuesService.Instance.CreateQuery()
    ...
    public class MProduct_FilterValuesModel
    {
        private int _Page = 0;
        public int Page
        {
