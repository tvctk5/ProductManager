...
{
    [ModuleInfo(Name = "MO : Product_ register email", Code = "MProduct_RegisterEmail", Order = 50)]
    public class MProduct_RegisterEmailController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_RegisterEmailModel model)
        {
        ...
        {
            var item = ModProduct_RegisterEmailService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_RegisterEmailService.Instance.CreateQuery()
    ...
    public class MProduct_RegisterEmailModel
    {
        private int _Page = 0;
        public int Page
        {
