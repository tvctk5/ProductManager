...
{
    [ModuleInfo(Name = "MO : Product_ city", Code = "MProduct_City", Order = 50)]
    public class MProduct_CityController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_CityModel model)
        {
        ...
        {
            var item = ModProduct_CityService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_CityService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MProduct_CityModel
    {
        private int _Page = 0;
        public int Page
        {
