...
{
    [ModuleInfo(Name = "MO : Product_ info_ area in national", Code = "MProduct_Info_AreaInNational", Order = 50)]
    public class MProduct_Info_AreaInNationalController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Info_AreaInNationalModel model)
        {
        ...
        {
            var item = ModProduct_Info_AreaInNationalService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Info_AreaInNationalService.Instance.CreateQuery()
    ...
    public class MProduct_Info_AreaInNationalModel
    {
        private int _Page = 0;
        public int Page
        {
