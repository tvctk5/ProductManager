...
{
    [ModuleInfo(Name = "MO : [Sản phẩm] Tỷ giá", Code = "MProduct_Currency", Order = 50)]
    public class MProduct_CurrencyController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_CurrencyModel model)
        {
        ...
        {
            var item = ModProduct_CurrencyService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_CurrencyService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MProduct_CurrencyModel
    {
        private int _Page = 0;
        public int Page
        {
