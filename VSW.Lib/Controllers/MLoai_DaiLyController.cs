...
{
    [ModuleInfo(Name = "MO : Loai_ dai ly", Code = "MLoai_DaiLy", Order = 50)]
    public class MLoai_DaiLyController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MLoai_DaiLyModel model)
        {
        ...
        {
            var item = ModLoai_DaiLyService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModLoai_DaiLyService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
