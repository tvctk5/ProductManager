...
{
    [ModuleInfo(Name = "MO : Product_ area", Code = "MProduct_Area", Order = 50)]
    public class MProduct_AreaController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_AreaModel model)
        {
        ...
        {
            var item = ModProduct_AreaService.Instance.CreateQuery()
                            .Where(o => o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_AreaService.Instance.CreateQuery()
    ...
    public class MProduct_AreaModel
    {
        private int _Page = 0;
        public int Page
        {
