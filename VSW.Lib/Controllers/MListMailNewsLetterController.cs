...
{
    [ModuleInfo(Name = "MO : Đăng ký nhận tin", Code = "MListMailNewsLetter", Order = 50)]
    public class MListMailNewsLetterController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MListMailNewsLetterModel model)
        {
        ...
        {
            var item = ModListMailNewsLetterService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModListMailNewsLetterService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MListMailNewsLetterModel
    {
        private int _Page = 0;
        public int Page
        {
