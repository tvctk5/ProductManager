...
{
    [ModuleInfo(Name = "MO : Product_ info_ size", Code = "MProduct_Info_Size", Order = 50)]
    public class MProduct_Info_SizeController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Info_SizeModel model)
        {
        ...
        {
            var item = ModProduct_Info_SizeService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Info_SizeService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MProduct_Info_SizeModel
    {
        private int _Page = 0;
        public int Page
        {
