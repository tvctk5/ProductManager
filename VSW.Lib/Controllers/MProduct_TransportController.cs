...
{
    [ModuleInfo(Name = "MO : Product_ transport", Code = "MProduct_Transport", Order = 50)]
    public class MProduct_TransportController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_TransportModel model)
        {
        ...
        {
            var item = ModProduct_TransportService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_TransportService.Instance.CreateQuery()
    ...
    public class MProduct_TransportModel
    {
        private int _Page = 0;
        public int Page
        {
