...
{
    [ModuleInfo(Name = "MO : Product_ info_ office", Code = "MProduct_Info_Office", Order = 50)]
    public class MProduct_Info_OfficeController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Info_OfficeModel model)
        {
        ...
        {
            var item = ModProduct_Info_OfficeService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Info_OfficeService.Instance.CreateQuery()
    ...
    public class MProduct_Info_OfficeModel
    {
        private int _Page = 0;
        public int Page
        {
