...
{
    [ModuleInfo(Name = "MO : Product_ order", Code = "MProduct_Order", Order = 50)]
    public class MProduct_OrderController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_OrderModel model)
        {
        ...
        {
            var item = ModProduct_OrderService.Instance.CreateQuery()
                            .Where(o => o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_OrderService.Instance.CreateQuery()
    ...
    public class MProduct_OrderModel
    {
        private int _Page = 0;
        public int Page
        {
