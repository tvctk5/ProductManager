...
{
    [ModuleInfo(Name = "MO : Product_ types", Code = "MProduct_Types", Order = 50)]
    public class MProduct_TypesController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_TypesModel model)
        {
        ...
        {
            var item = ModProduct_TypesService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_TypesService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MProduct_TypesModel
    {
        private int _Page = 0;
        public int Page
        {
