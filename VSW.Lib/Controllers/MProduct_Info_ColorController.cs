...
{
    [ModuleInfo(Name = "MO : Product_ info_ color", Code = "MProduct_Info_Color", Order = 50)]
    public class MProduct_Info_ColorController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Info_ColorModel model)
        {
        ...
        {
            var item = ModProduct_Info_ColorService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Info_ColorService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
    ...
    public class MProduct_Info_ColorModel
    {
        private int _Page = 0;
        public int Page
        {
