...
{
    [ModuleInfo(Name = "MO : Quận huyện", Code = "MDistrict", Order = 50)]
    public class MDistrictController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MDistrictModel model)
        {
        ...
        {
            var item = ModDistrictService.Instance.CreateQuery()
                            .Where(o => o.Activity == true && o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModDistrictService.Instance.CreateQuery()
                                        .Where(o => o.Activity == true)
