...
{
    [ModuleInfo(Name = "MO : Parameters", Code = "MParameters", Order = 50)]
    public class MParametersController : Controller
    {
        ...
        public void ActionIndex(MParametersModel model)
        {
        ...
        {
            var item = ModParametersService.Instance.CreateQuery()
                            .Where(o => o.Code == endCode)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModParametersService.Instance.CreateQuery()
