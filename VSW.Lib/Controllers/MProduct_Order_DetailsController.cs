...
{
    [ModuleInfo(Name = "MO : Product_ order_ details", Code = "MProduct_Order_Details", Order = 50)]
    public class MProduct_Order_DetailsController : Controller
    {

        [VSW.Core.MVC.PropertyInfo("Số lượng")]
        public int PageSize = 10;

        public void ActionIndex(MProduct_Order_DetailsModel model)
        {
        ...
        {
            var item = ModProduct_Order_DetailsService.Instance.CreateQuery()
                            .Where(o => o.ID == id)
                            .ToSingle();

            if (item != null)
            {
                ViewBag.Other = ModProduct_Order_DetailsService.Instance.CreateQuery()
    ...
    public class MProduct_Order_DetailsModel
    {
        private int _Page = 0;
        public int Page
        {
