...
namespace VSW.Lib.CPControllers
{
    ...
    public class FormProduct_FilterGroupsController : CPController
    {
        public FormProduct_FilterGroupsController()
        {
            //khoi tao Service
        ...
        private bool ValidSave(ModProduct_FilterGroupsModel model)
        {
            ...
            if (CPViewPage.Message.ListMessage.Count == 0)
            {
                ...
                if(false) //(ModProduct_FilterGroupsService.Instance.DuplicateCode(item.Code, model.RecordID, ref sMessError))
                {
                    if (string.IsNullOrEmpty(sMessError))
                        CPViewPage.Message.ListMessage.Add(CPViewControl.ShowMessDuplicate("Mã nhóm thuộc tính lọc", item.Code));
                    else
