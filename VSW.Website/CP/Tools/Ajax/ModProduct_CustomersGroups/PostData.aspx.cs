...
namespace VSW.Website.CP.Tools.Ajax.Common.ModProduct_CustomersGroups
{
    partial class PostData : System.Web.UI.Page
    {
        ...
        private void CheckDuplicate()
        {
            ...
            if (ModProduct_CustomersGroupsService.Instance.DuplicateCode(Code, RecordID, ref sMessError))
            {
                if (string.IsNullOrEmpty(sMessError))
                {
                    objDataOutput.NotDuplicate = false;
                    objDataOutput.MessSuccess = CPViewControl.ShowMessDuplicate("Mã nhóm khách hàng", Code);
                }
                else
                {
