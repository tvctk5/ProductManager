...
namespace VSW.Website.CP.Tools.Ajax.Common.ModProduct_Office
{
    partial class PostData : System.Web.UI.Page
    {
        ...
        private void CheckDuplicate()
        {
            ...
            if (ModProduct_OfficeService.Instance.DuplicateCode(Code, RecordID, ref sMessError))
            {
                if (string.IsNullOrEmpty(sMessError))
                {
                    objDataOutput.NotDuplicate = false;
                    objDataOutput.MessSuccess = CPViewControl.ShowMessDuplicate("Mã siêu thị/ văn phòng", Code);
                }
                else
                {
