...
namespace VSW.Website.CP.Tools.Ajax.Common.ModProduct_Manufacturer
{
    partial class PostData : System.Web.UI.Page
    {
        ...
        private void CheckDuplicate()
        {
            ...
            if (ModProduct_ManufacturerService.Instance.DuplicateCode(Code, RecordID, ref sMessError))
            {
                if (string.IsNullOrEmpty(sMessError))
                {
                    objDataOutput.NotDuplicate = false;
                    objDataOutput.MessSuccess = CPViewControl.ShowMessDuplicate("Mã nhà sản xuất", Code);
                }
                else
                {
