...
namespace VSW.Website.CP.Tools.Ajax.Common.ModProduct_Supplier
{
    partial class PostData : System.Web.UI.Page
    {
        ...
        private void CheckDuplicate()
        {
            ...
            if (ModProduct_SupplierService.Instance.DuplicateCode(Code, RecordID, ref sMessError))
            {
                if (string.IsNullOrEmpty(sMessError))
                {
                    objDataOutput.NotDuplicate = false;
                    objDataOutput.MessSuccess = CPViewControl.ShowMessDuplicate("Mã nhà cung cấp", Code);
                }
                else
                {
