...
<script runat="server">
    ...
    public Permissions Permissions
    {
        get
        {
            ...
            try
            {
                string suser = Request.QueryString[VSW.Core.Web.Setting.Sys_SiteID + "_CP.UserID"].Replace(" ", "+");
                string IP = HttpContext.Current.Request.UserHostAddress;
                string suserid = VSW.Core.Global.CryptoString.Decrypt(suser).Replace(IP + "_VSW_" + VSW.Core.Web.Setting.Sys_SiteID + "_CP.UserID", string.Empty);
                int _UserID = VSW.Core.Global.Convert.ToInt(suserid);
	...
	public override void SetConfig()
	{
        ...
        type = ResourceType.Add("Resources");
        type.Url = BaseUrl + "resources/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "resources/";
        type.MaxSize = 0;
        //type.AllowedExtensions = new string[] { "css", "js", "map","jpg","png","gif", "icon" };
        type.DeniedExtensions = new string[] { };

        /*type = ResourceType.Add("Css");
        type.Url = BaseUrl + "resources/css/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "resources/css/";
        type.MaxSize = 0;
        type.AllowedExtensions = new string[] { "css" };
        type.DeniedExtensions = new string[] { };

        type = ResourceType.Add("Js");
        type.Url = BaseUrl + "resources/js/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "resources/js/";
        type.MaxSize = 0;
        type.AllowedExtensions = new string[] {"js", "map" };
        type.DeniedExtensions = new string[] { };*/
	}
