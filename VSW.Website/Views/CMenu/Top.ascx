        ...
        <%for (int i = 0; listParent != null && i < listParent.Count; i++)
          {
              ...
              var link = string.IsNullOrEmpty(listParent[i].Url) ? ViewPage.GetPageURL(listParent[i].SysPageID, listAllPage) : listParent[i].Url;
              var listChilden = listItem.Where(o => o.ParentID == listParent[i].ID).ToList();
     %>
        <li><a class="color<%=i %>" href="<%=link %>"><%=listParent[i].Name%></a>

        <% if (listChilden != null && listChilden.Count > 0)
           {%>
           <div class="megapanel">
					<div class="row">
                    <% for (int j = 0; j < listChilden.Count; j++)
                       {
                           link = string.IsNullOrEmpty(listChilden[j].Url) ? ViewPage.GetPageURL(listChilden[j].SysPageID, listAllPage) : listChilden[j].Url;
                           var page = listAllPage.Where(o=>o.ID == listChilden[j].SysPageID).SingleOrDefault();
                           var listProduct_sub = new List<ModProduct_InfoEntity>();
                           if(page != null)
                           {
                                listProduct_sub = listProduct.Where(o=>o.MenuID == page.MenuID).ToList();
                           }
                           %>
                           <div class="col-md-2">
                               <div class="h_nav">
								    <h5 class="top" style="font-weight:bold;"><a href="<%=link %>"><%= listChilden[j].Name %></a></h5>
								    <ul>
                                    <% foreach (var item_Product in listProduct_sub)
                                       {
                                           string ProductUrl = ViewPage.GetURL(item_Product.MenuID, item_Product.Code);
                                        %>
                                         <li><a href="<%= ProductUrl %>"><%= item_Product.Name %></a></li>  
                                       <%} %>
